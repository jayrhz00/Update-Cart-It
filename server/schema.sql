...
-- TABLE 7: user_store_coupons — promo codes the user saves, keyed by store for checkout

CREATE TABLE IF NOT EXISTS user_store_coupons
(
    ...
    discount_label VARCHAR(300),
    fine_print TEXT,
    expires_at TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT fk_user_store_coupons_user
        FOREIGN KEY (user_id)
        REFERENCES users(user_id)
        ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_user_store_coupons_user ON user_store_coupons(user_id);
CREATE INDEX IF NOT EXISTS idx_user_store_coupons_store ON user_store_coupons(user_id, store_name);

-- TABLE 8: coupon_usage_events — feedback loop for ranking "top coupons today"
...
(
    ...
    source VARCHAR(20),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT fk_coupon_usage_events_user
        FOREIGN KEY (user_id)
        REFERENCES users(user_id)
        ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_coupon_usage_events_created_at
    ON coupon_usage_events(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_coupon_usage_events_store_code
    ON coupon_usage_events(store_name, code);
